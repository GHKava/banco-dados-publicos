...
2. **Política de fontes (Allowlist)**
   ...
   - **Onboarding obrigatório (sem isso, a fonte NÃO entra):**
     ...
     - **Rate/budget por domínio:** definir _crawl_delay/rps/concurrency_ e _pages_per_day/bytes_per_day_ (evita “acidentalmente virar DDoS”).
...
**REGRA:** Nenhuma fonte pode ser ingerida sem passar pelo onboarding completo e estar registrada nos arquivos abaixo.

### Arquivos obrigatórios (nada fica perdido)

1. **Fonte de verdade operacional:**
   - **Arquivo:** `configs/sources.yaml`
   - **Objetivo:** Configuração técnica lida por bots (source_id, domains, rate_limits, discovery method, policy gate decision)
   - **Formato:** YAML estruturado com todas as 5 fontes aprovadas (SRC-001 a SRC-005) + futuras adições via CR
   - **Regra:** Somente fontes listadas aqui podem ser ingeridas. Se não está no YAML → BLOCK.

2. **Documentação auditável (humano-primeiro):**
   - **Arquivo:** `_OBSIDIAN/Organização do Projeto/Fontes_Licencas.md`
   - **Objetivo:** Registro humano de licenças, decisões de policy gate, compliance, PII esperado
   - **Conteúdo:** Tabela de todas as fontes (APROVADAS, REJEITADAS, CANDIDATAS) com status de licença, ToS, robots.txt, decisão de armazenamento (ALLOW_FULLTEXT | METADATA_ONLY | SNIPPETS_ONLY | BLOCK)
   - **Regra:** Evidência de licença obrigatória. Sem evidência → METADATA_ONLY (fail-closed).

3. **Template obrigatório para onboarding:**
   - **Arquivo:** `_OBSIDIAN/Organização do Projeto/Templates/Template - Source Onboarding Checklist.md`
   - **Objetivo:** Checklist padrão com 10 seções (Identificação, Discovery, Compliance, Policy Gate, Rate Limits, Budget, Testes, Riscos, Evidence Pack, Aprovação)
   - **Uso:** Base para todas as notas de onboarding individual por fonte. Nunca modificar diretamente; copiar e preencher.

4. **Checklists preenchidos por fonte (onboarding individual):**
   - **Localização:** `_OBSIDIAN/Organização do Projeto/Onboarding/SRC-XXX - <Nome da Fonte>.md`
   - **Conteúdo:** Checklist preenchido com:
     - Robots.txt verificado? (URL, data, status)
     - ToS verificado? (URL, screenshot, permite scraping?)
     - Licença identificada? (tipo, URL, permite fulltext?)
     - Policy gate decision (METADATA_ONLY | ALLOW_FULLTEXT | BLOCK)
     - Rate limits definidos (rps, concurrency, budget)
     - PII esperado? (detecção obrigatória)
     - Golden docs selecionados (3+ URLs de teste)
     - Evidence pack criado (robots.txt copy, screenshots, logs)
   - **Regra:** 1 arquivo por fonte. Status IN_REVIEW → compliance verificado → APROVADO → entrada em sources.yaml.

5. **Evidence Packs (prova de compliance):**
   - **Localização:** `docs/evidence/T-ONB-SRC-XXX/`
   - **Arquivos obrigatórios:**
     - `robots.txt` (cópia do robots.txt da fonte)
     - `tos_screenshot.png` (screenshot de ToS/licença)
     - `license_screenshot.png` (screenshot de página de licença)
     - `sample_urls.txt` (3+ URLs de teste)
     - `fetch_test.log` (log de teste de fetch com sucesso)
     - `notes.md` (resumo de decisões e observações)
   - **Regra:** Evidence pack obrigatório para aprovar fonte. Sem evidência → METADATA_ONLY.

### Policy Gate (fail-closed sempre)

- **Sem licença verificada → METADATA_ONLY** (nunca armazenar fulltext por padrão)
- **Licença verificada + compatível (CC-BY, LAI, etc.) → ALLOW_FULLTEXT** (upgrade após compliance)
- **Licença restritiva ou ToS proíbe scraping → BLOCK** (não ingerir)
- **PII esperado → detecção obrigatória** (presidio-analyzer sempre ativo)

### Processo de onboarding (workflow completo)

1. **Ideia de fonte** → registrar em `_OBSIDIAN/Organização do Projeto/Banco de Ideias.md` (triagem)
2. **Triagem aprovada** → PM/LEGAL aprovam candidatura
3. **Criar onboarding note** → copiar template para `Onboarding/SRC-XXX - <Nome>.md`
4. **Verificar compliance** → Robots.txt + ToS + Licença (screenshots obrigatórios)
5. **Policy gate decision** → METADATA_ONLY | ALLOW_FULLTEXT | BLOCK (documentar justificativa)
6. **Criar evidence pack** → `docs/evidence/T-ONB-SRC-XXX/` (6 arquivos mínimos)
7. **Registro duplo:**
   - Adicionar entrada em `configs/sources.yaml` (config técnica)
   - Adicionar entrada em `Fontes_Licencas.md` (registro auditável)
8. **Aprovação final** → PM + LEGAL + DPO assinam
9. **Commit** → Git commit com mensagem "feat: onboarding SRC-XXX completo + evidence pack"

### Fontes iniciais (MVP — DEC-008)

**Decisão:** 5 fontes governamentais brasileiras com policy fail-closed:

1. **SRC-001:** Diário Oficial da União (DOU) - in.gov.br (METADATA_ONLY até licença verificada)
2. **SRC-002:** Planalto - Legislação - planalto.gov.br (METADATA_ONLY até licença verificada)
3. **SRC-003:** IBGE - APIs e Dados - ibge.gov.br (METADATA_ONLY; upgrade para ALLOW_FULLTEXT se CC-BY confirmado)
4. **SRC-004:** Banco Central - Dados Abertos - dadosabertos.bcb.gov.br (METADATA_ONLY até licença verificada)
5. **SRC-005:** dados.gov.br - Catálogo (METADATA_ONLY; licença VARIÁVEL POR DATASET; verificar individualmente)

**Regra especial para SRC-005:** Criar tabela de tracking de datasets individuais (`docs/evidence/T-ONB-SRC-005/license_tracking_table.csv`) com colunas: dataset_id, dataset_name, license_id, license_verified, storage_mode, pii_detection_required, verified_date.

### Revisão e auditoria

**Frequência:** Trimestral (ou ao adicionar nova fonte)  
...
- [ ] PII detection rodando em fontes com PII esperado?
